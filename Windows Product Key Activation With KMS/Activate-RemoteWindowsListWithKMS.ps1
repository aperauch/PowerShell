$INPUT_FILE = "C:\path\to\Windows_Servers_List.txt"

# Get creds if they don't exist already to establish remote PS session
if (!$creds) { 
    $creds = Get-Credential -Message "Enter your EUC admin credentials.  Example username: euc\jsmith_admin"
}
